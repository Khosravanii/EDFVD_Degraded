...
def show_Tasks(Task_show,n,X):
    ...
    for i in range(n):
        if tasks_show[i][5] >0:
            tasks_show[i].append(round(X * tasks[i][3],2))
        else:
            tasks_show[i].append('-')
    return tasks_show
...
def utilization(Task_Set,Len_Task_Set,X):
    ...
    for i in range(n):
        if show[i][5]==0:
            u_lo_lo +=show[i][1]/show[i][4]
            u_hi_lo +=show[i][2]/show[i][4]
        elif show[i][5]==1:
            u_hi +=show[i][2]/show[i][4]
            u_lo_hi +=show[i][1]/show[i][4]
        else:
            break
...
def EDF_VD(tasks,n,lcm,util):
    ...
    for i in range(n):
        j=0
        while 1:
            if tasks[i][3]+j*tasks[i][4]<=lcm:
                instances.append([[tasks[i][0], tasks[i][1], tasks[i][2],tasks[i][3]+j*tasks[i][4],(j+1)*tasks[i][3]  , tasks[i][5],0],j*tasks[i][3]])
                j+=1
                #f tasks[i][5]==1:
            ...
            while time<lcm:
                ...
                for j in range(len(instances)):
                    ...
                    if anyrun==1 and instances[j][0][5]>0 and timeLeft[instances[j][0][0]-1]==0 and criticality ==0:
                        signal=int(input("Task ("+str(instances[j][0][0])+") Time ("+str(time+1)+") signal %d/0:"%(sig)))
